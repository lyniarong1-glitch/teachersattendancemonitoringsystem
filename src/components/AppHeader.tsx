    ...
    <header className="no-print border-b border-sidebar-border bg-sidebar text-sidebar-foreground">
      <div className="mx-auto flex min-h-16 max-w-[95rem] items-center justify-between gap-3 px-3 py-2 sm:px-4">
        ...
        <div className="flex shrink-0 items-center gap-1 sm:gap-2">
          ...
          <Sheet><SheetTrigger asChild><Button size="icon" variant="secondary" aria-label="Open navigation menu"><Menu className="h-5 w-5" /></Button></SheetTrigger><SheetContent className="flex flex-col"><SheetHeader><SheetTitle>Account Menu</SheetTitle><SheetDescription>{name} · {role}</SheetDescription></SheetHeader><nav className="mt-6 grid gap-2"><SheetClose asChild><Button variant="ghost" className="justify-start" onClick={() => void navigate({ to: isSA ? "/sa" : "/hr" })}><Home className="mr-3 h-4 w-4" />Dashboard</Button></SheetClose><SheetClose asChild><Button variant="ghost" className="justify-start" onClick={() => setProfileOpen(true)}><UserRound className="mr-3 h-4 w-4" />My Profile</Button></SheetClose></nav><div className="mt-auto grid gap-2 border-t pt-4"><Button variant="outline" className="justify-start" onClick={() => void signOut()}><LogOut className="mr-3 h-4 w-4" />Sign Out</Button>{isSA && <Button variant="destructive" className="justify-start" onClick={() => setDeleteOpen(true)}><Trash2 className="mr-3 h-4 w-4" />Delete Account</Button>}</div></SheetContent></Sheet>
        </div>
      </div>
    </header>
